...
let make = (~modalId, ~onClose, ~_in=true, ~onExited=() => ()) => {
  ...
  let (submissionStatus, setSubmissionStatus) = useState(() => JobOfferForm.Pristine);
  let onSubmit =
    useCallback1(
      (values: FormValidations.values(string)) => {
        setSubmissionStatus(_status => JobOfferForm.Pending);
        ...
        let _promise =
          Js.Promise.(
            Fetch.fetchWithInit(
              "/.netlify/functions/sendJobOfferEmail",
  ...
  let onChange =
    useCallback2(
      (_values, _eventMetadata) => {
        ...
        | JobOfferForm.Failed(_) => setSubmissionStatus(_status => JobOfferForm.Pristine)
        | _ => ()
        }
      },
    ...
    <ScrollContainer>
      <div className=Styles.contacts>
        ...
        <div className=Styles.form>
          ...
          <JobOfferForm className=formClassName onSubmit onChange submissionStatus />
        </div>
      </div>
    </ScrollContainer>
    <Header
